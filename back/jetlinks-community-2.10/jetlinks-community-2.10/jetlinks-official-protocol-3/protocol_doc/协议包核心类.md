# 认证器
认证器`(Authenticator)`是用于在收到设备请求(例如MQTT)时，对客户端进行认证时使用，不同的网络协议
...
参数`AuthenticationRequest`为认证请求参数,不同的网络类型请求类型也不同，请根据实际情况转换为对应的类型，例如:`MqttAuthenticationRequest mqttRequest = (MqttAuthenticationRequest)request;`

参数`DeviceOperator`为对应的设备操作接口,可通过此接口获取设备的配置，例如：

`device.getConfig("mqttUsername")`。
...
# 上下文
## 编码上下文结构
具体参考源码：[MessageEncodeContext](https://github.com/jetlinks/jetlinks-core/blob/master/src/main/java/org/jetlinks/core/message/codec/MessageEncodeContext.java)。
...
## 解码上下文结构
具体参考源码：[MessageDecodeContext](https://github.com/jetlinks/jetlinks-core/blob/master/src/main/java/org/jetlinks/core/message/codec/MessageDecodeContext.java)。
...
大部分情况下:`MessageDecodeContext`可转为`FromDeviceMessageContext`,可获取到当前设备的连接

会话`DeviceSession`,通过会话可以直接发送消息到设备。
...
具体参考源码：[EncodedMessage](https://github.com/jetlinks/jetlinks-core/blob/master/src/main/java/org/jetlinks/core/message/codec/EncodedMessage.java)。
...
消息拦截器更多详情可参考文档：[消息拦截器](https://hanta.yuque.com/px7kg1/nn1gdr/ymfpop6hbuumen6o)。

:::

# 配置元数据
配置元数据用于告诉平台，在使用此协议的时候，针对特定的消息传输方式，需要添加一些自定义配置到设备配置`(DeviceOperator.setConfig)`中。在其他地方可以通过`DeviceOperator.getConfig`获取这些配置。
...
基本信息可在ProtocolSupportProvider的实现类中的create方法里添加。通过返回对象ProtocolSupport中的方法添加具体配置。
...
## 配置markdown文档
可以在协议包`src/main/resources`目录中添加markdown文档，然后配置到`ProtocolSupport`中。例如：
...
| **定义的类**                              | **描述**                                                     |
| ----------------------------------------- | ------------------------------------------------------------ |
| `DeviceFeatures.supportFirmware`          | 通过此特性判断是否支持远程升级。协议包添加此特性后，使用此协议包的产品才会出现在平台的[远程升级](https://hanta.yuque.com/px7kg1/yfac2l/ok2h3v8yhl4ogrtg)产品下拉选择中 |
| `CodecFeature.transparentCodec`           | 通过此特性判断是否支持透传。产品和设备详情根据此特性判断是否显示[数据解析](https://hanta.yuque.com/px7kg1/yfac2l/nrngqtbpp107xguw#hDxPj)页面 |
| `MetadataFeature`                         | 控制物模型是否支持增删改，以及是否支持派生。                 |
| `MetadataFeature.diffMetadataSameProduct` | 将插件/协议包中的物模型属性标识与平台物模型属性标识进行映射，参见：[物模型映射](https://hanta.yuque.com/px7kg1/nn1gdr/ov1a3nds3b9mn6hz) |
...
# 设备自注册
自定义协议包将消息解析为`DeviceRegisterMessage`并设置header头，参数如下：

| 设备自注册参数 | 必选 | 类型               |
| -------------- | ---- | ------------------ |
| productId      | 是   | String             |
| deviceName     | 是   | String             |
| configuration  | 否   | Map<String,Object> |


平台将自动添加设备信息到设备实例中。

如果是注册子设备,则解析为 `ChildDeviceMessage<DeviceRegisterMessage>`。

:::info
**<font style="color:#2F4BDA;">🌏</font>****<font style="color:#2F4BDA;"> 说明</font>**

header中的`configuration`为设备的自定义配置信息，默认会保存到Redis中并存储在`device:{deviceId}`里。

在协议包中可通过`DeviceOperator.getSelfConfig`来获取这些配置信息。
...
return context.getDevice(deviceId)
	.flatMap(operator-> operator.getConfigs("keyName"))
	.map(values -> {
		String key = values.getString("keyName","默认值");
		//对key做业务逻辑处理
		doSomething(key);
...
# 完整例子
[官方协议](https://github.com/jetlinks/jetlinks-official-protocol)

---

| 上一篇：[官方协议TCP报文](https://hanta.yuque.com/px7kg1/nn1gdr/atpog2hc57l9fihz) | 下一篇：[协议调试](https://hanta.yuque.com/px7kg1/nn1gdr/ops3g4ouem7ip0qw) |
| :--- | ---: |