...
该功能及API在 <font style="color:#DF2A3F;">jetlinks 1.4.0</font> 后提供。

默认情况下使用tcp和mqtt方式接入时,当连接断开时,则认为设备已离线。

但是在某些场景(如:低功率设备)下,无法使用长连接进行通信,可以通过指定特定配置使平台保持设备在线状态。

短连接接入场景：HTTP，MQTT/TCP发完即断开连接，CoAP协议，真实设备断电情况。
...
1. MQTT接入时添加到任意消息即可。TCP接入时添加到`DeviceOnlineMessage`即可。
2. 如果服务重启，将不会保持在线状态！
3. 需要在`JetLinks`后端服务中添加额外参数
+ 源码启动：**启动时添加参数**`**-Ddevice.session.keep-online.ignore-parent=true**` 。
+ `docker`启动：在`docker-compose`文件中`jetlinks`服务的`JAVA_OPTS`参数中添加`-Ddevice.session.keep-online.ignore-parent=true`。
...
### 网关子设备
此方式使用可能会造成父子设备的状态不一致：父设备状态取决于socket 连接状态，而子设备是以发送消息频率来更新设备状态（最大不超过 2 倍的`keepOnlineTimeoutSeconds`值），如父设备连接断开（离线），而子设备还在保活时间内（在线）。

```java
//设置让会话强制在线
message.addHeader(Headers.keepOnline, true);
//保持在线,忽略父设备状态,强制以心跳超时来决定是否离线.
message.addHeader(Headers.keepOnlineIgnoreParent, true);
//设置超时时间（可选,默认10分钟），如果超过这个时间没有收到任何消息则认为离线。
message.addHeader(Headers.keepOnlineTimeoutSeconds, 600);
```
...
在进行消息下发时，因为会话是强制保持在线的，所以消息会直接通过session下发，但是此时设备可能已经断开了连接, 将会抛出异常`DeviceOperationException(ErrorCode.CLIENT_OFFLINE)`。

此时可以在发送消息后拦截异常将消息缓存起来，等待下次设备连接上来后再下发指令。
...
public class JetLinksProtocolSupportProvider implements ProtocolSupportProvider {
	...
	public <R extends DeviceMessage> Flux<R> afterSent(DeviceOperator device, DeviceMessage message, Flux<R> reply) {
		...
		return reply.onErrorResume(DeviceOperationException.class, err -> {
			//处理异常状态码是离线的情况，其他异常情况仍然抛出异常信息
			if (err.getCode() == ErrorCode.UNSUPPORTED_MESSAGE || err.getCode() == ErrorCode.TIME_OUT) {

				return webClient
					.get()
					.uri("http://192.168.32.16:8000/api/system/version")
					.retrieve()
					.bodyToFlux(JSONObject.class)
					.thenMany(Flux.just(((RepayableDeviceMessage<?>) message)
											.newReply()
											.code(ErrorCode.REQUEST_HANDLING.name())
											.message("设备处理中...")
											.success()))
...
| `keepOnline`                 | `HeaderKey<Boolean>` | true                                               | 保持在线,与DeviceOnlineMessage配合使用.                      |
| `keepOnlineIgnoreConnection` | `HeaderKey<Boolean>` | false                                              | 在保持在线时,忽略连接状态信息,设备是否在线以: keepOnlineTimeoutSeconds指定为准 |
| `keepOnlineTimeoutSeconds`   | `HeaderKey<Integer>` | 600                                                | 保持在线超时时间,超过指定时间未收到消息则认为离线，单位：秒  |
| `async`                      | `HeaderKey<Boolean>` | false                                              | 异步消息,当发往设备的消息标记了为异步时,设备网关服务发送消息到设备后将立即回复org.jetlinks.core.enums.ErrorCode.REQUEST_HANDLING到发送端 |
...
| `enableTrace`                | `HeaderKey<Boolean>` | Boolean.getBoolean("device.message.trace.enabled") | 是否开启追踪,开启后header中将添加各个操作的时间戳，参数可以在application.yml里面指定 |
...
| `propertyContainsGeo`        | `HeaderKey<Boolean>` | true                                               | 上报属性中是否包含geoPoint信息,如果设置为false,上报属性时则不处理地理位置相关逻辑,可能提高一些性能 |
| `geoProperty`                | `HeaderKey<Boolean>` | null                                               | 明确定义上报属性中包含的geo属性字段,在设备物模型属性数量较大时有助于提升地理位置信息处理性能，填写物模型定义为geoPoint类型的标识 |
| `clearAllSession`            | `HeaderKey<Boolean>` | false                                              | 在设备离线时,标记是否清理所有会话.   通常用于短连接方式接入平台的场景, 在集群的多台节点中存在同一个设备的会话时,默认只有集群全部会话失效时,设备才算离线.   可通过在发送离线消息中指定header:clearAllSession来标识是否让集群全部会话都失效 |



---

| 上一篇：[主动从设备获取属性和事件](https://hanta.yuque.com/px7kg1/pifo7x/hb6udmyrpxy2nlxh) | 下一篇：[设备自注册](https://hanta.yuque.com/px7kg1/nn1gdr/zqowgg1qc1sl4gwr) |
| :--- | ---: |