...
const names = {
  network: `codex-mqtt-network-${stamp}`,
  access: `codex-mqtt-access-${stamp}`,
  product: `codex-mqtt-product-${stamp}`,
  device: `codex-mqtt-device-${stamp}`,
  clientId: `codex-client-${stamp}`,
}
...
if (await inputs.count()) await inputs.nth(0).fill(names.access)
const protocolSelect = page.locator('.step-panel').nth(1).locator('.ant-select').last()
console.log('PROTOCOL_SELECT_COUNT', await protocolSelect.count())
if (await protocolSelect.count() && !(await protocolSelect.locator('.ant-select-selection-item').count())) {
  console.log('PROTOCOL_OPEN')
  await protocolSelect.evaluate(element => element.click())
  await page.waitForTimeout(300)
  console.log('PROTOCOL_OPTIONS', await page.locator('.ant-select-dropdown:not(.ant-select-dropdown-hidden) .ant-select-item-option').count())
  ...
  console.log('PROTOCOL_SELECTED')
}
console.log('BEFORE_STEP1_NEXT')
await clickLastContentButton()
console.log('AFTER_STEP1_NEXT')

console.log('BEFORE_STEP2_RADIO')
await clickRadio(2)
console.log('AFTER_STEP2_RADIO')
inputs = page.locator('.step-panel').nth(2).locator('input:visible')
console.log('STEP2_INPUTS', await inputs.count())
console.log('STEP2_VALUES', await inputs.evaluateAll(els => els.map(e => ({ type: e.type, placeholder: e.placeholder, value: e.value }))))
...
await page.screenshot({ path: 'front/tests/e2e/mqtt-closure-final.png', fullPage: true })
